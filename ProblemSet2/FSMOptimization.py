...
# Plan
'''
1 -> Find the live states
    -> Find the states
    -> Run nfsmaccepts at each and every stage

2 -> Create new FSM
    -> For each state/edge:
      - if dead, don't copy
      - else:
        - copy dests over if live
        - if no live dest., don't copy
    - update accepting
'''


def nfsmtrim(edges, accepting): 
    # Gathering up all states
    states = [ ]
    for e in edges:
      states = states + [e[0]] + edges[e]
    # Find out live states
    live = [ ]
    for s in states:
      if nfsmaccepts(s, edges, accepting, [ ]) != None:
        live = live + [s]
    # Building Optimal FSM
    new_edges = { }
    for e in edges:
      if e[0] in live:
        new_destinations = [ ]
        for destinations in edges[e]:
          if destinations in live:
            new_destinations = new_destinations + [destinations]
        if new_destinations != []:
